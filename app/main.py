...
async def lifespan(app: FastAPI):
    ...
    logger.info("All componets initialized. Redy to serve requests.")
...
# === Rate Limiter (Tracks request per ip address) === #
...
app = FastAPI(
    ...
    description="A production-redy chat API with securing caching and observablity.",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def chat(request: Request, body: ChatRequest):
    ...
    Main chat endpoints.
    Flow :
    1. Security check (Injestion + PII masking)
    2. cache Lookup
...
async def health():
    """Health ckeck for docker/kubernatics."""
